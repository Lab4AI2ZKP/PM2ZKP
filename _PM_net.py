...
net = VAE() # 实例化网络
net.load_state_dict(torch.load('D:/exp/even/net_60000_97.11'))
...
train_set = MNIST('D:\mnist', train=False, transform=im_tfs,  download=True)
train_data = DataLoader(train_set, batch_size = 128, shuffle=True, drop_last=True) 
validate_set = MNIST('D:\mnist', train=True, transform=im_tfs,  download=True)
...
for i in range (1):
    ...
    for e in range(1):
        ...
        for im, _ in train_data:
            ...
            MSE_loss_train = MSE_loss_train + torch.sum(MSE).detach().numpy() / im.shape[0]
            KLD_loss_train = KLD_loss_train + KLD.detach().numpy() / im.shape[0]
        ...
        MSE_train_losses.append(MSE_loss_train / len(train_data))
        KLD_train_losses.append(KLD_loss_train / len(train_data))
        ...
        print('epoch: {}, Loss: {:.4f}'.format(i + 1, train_loss/len(train_data)))
        for im, _ in validate_data:
            ...
            MSE_loss_valid = MSE_loss_valid + torch.sum(MSE).detach().numpy() / im.shape[0]
            KLD_loss_valid = KLD_loss_valid + KLD.detach().numpy() / im.shape[0]
        _valids.append(_valid_value / len(validate_data))
        MSE_valid_losses.append(MSE_loss_valid / len(validate_data))
        KLD_valid_losses.append(KLD_loss_valid / len(validate_data))
        print('epoch: {}, MSE_loss: {:.4f}, KLD_loss: {:.4f}, valid: {:.4f}, _valid: {:.4f}, dich_loss: {:.4f}, dich_value: {:.4f}, mu_std: {:.4f}, std_mean: {:.4f}'.format(i + 1, MSE_loss_train / len(train_data), KLD_loss_train / len(train_data), valid_value / len(train_data), _valid_value / len(validate_data), dich_loss / len(train_data), dich_value / len(train_data), mu_std / len(train_data), std_mean / len(train_data)))


#torch.save(net.state_dict(), 'D:/exp/even/net_50_1nn_97.51')
name = ['train_loss', 'MSE_train_loss', 'KLD_train_loss', 'MSE_valid_loss', 'KLD_valid_loss', 'dich_loss', 'dich_value', 'mu_std', 'std_mean', 'valid_train', 'valid_test'] 
log = pd.DataFrame(columns = name)
log['train_loss'] = train_losses
log['MSE_train_loss'] = MSE_train_losses
log['KLD_train_loss'] = KLD_train_losses
log['MSE_valid_loss'] = MSE_valid_losses
log['KLD_valid_loss'] = KLD_valid_losses
...
log.to_csv('D:/exp/even/log_60000.csv')
#log.to_csv('D:/exp/try/log_50_1_100_up.csv')
 


net.load_state_dict(torch.load('D:/exp/even/net_60000_97.11'))
test_set = MNIST('D:\mnist', train=True, transform=im_tfs,  download=True)
...
eps = torch.zeros(len(test_set), 50)
...
log = pd.read_csv('D:/exp/even/log_60000.csv')
...
plt.xlabel('loop index of training 10000 samples',  fontdict={'family' : 'Times New Roman', 'size': 14})
plt.ylim(100, 2500)
